          ...
          <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
            <DialogContent className="sm:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Share this carpool</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const subject = `Join ${carpool.parentName}'s Carpool for ${partyGroup?.name || 'the event'}`;
                        const body = `Hi!\n\nI'm organizing a carpool for ${partyGroup?.name || 'the event'} and I have spaces available.\n\nEvent Details:\n${partyGroup?.eventAddress ? `Location: ${partyGroup.eventAddress}, ${partyGroup.eventCity}, ${partyGroup.eventPostcode}` : ''}\n${partyGroup?.eventDate ? `Date: ${partyGroup.eventDate}` : ''}\n${partyGroup?.targetArrivalTime ? `Time: ${partyGroup.targetArrivalTime}` : ''}\n\nClick here to request a spot in my carpool:\n${window.location.origin}/join-carpool?partyGroupId=${partyGroupId}&carpoolId=${carpool.id}\n\nThanks!\n${carpool.parentName}`;
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const shareUrl = `${window.location.origin}/join-carpool?partyGroupId=${partyGroupId}&carpoolId=${carpool.id}`;
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const shareText = `Join my carpool for ${partyGroup?.name || 'the event'}! I have spaces available.`;
                        const shareUrl = `${window.location.origin}/join-carpool?partyGroupId=${partyGroupId}&carpoolId=${carpool.id}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        <TabsContent value="to-event" className="mt-4 space-y-6">
          {/* Map View */}
          <div className="space-y-4">
            <GoogleMap
              ...
              carpoolLocations={carpools?.filter((c: any) => c.canPickup || c.canBoth).map(carpool => ({
                id: carpool.id,
        ...
        <TabsContent value="from-event" className="mt-4 space-y-6">
          {/* Map View */}
          <div className="space-y-4">
            <GoogleMap
              className="w-full h-96"
        ...
        <TabsContent value="round-trip" className="mt-4 space-y-6">
          {/* Map View */}
          <div className="space-y-4">
            <GoogleMap
              className="w-full h-96"
        ...
        <TabsContent value="both" className="mt-4 space-y-6">
          {/* Map View */}
          <div className="space-y-4">
            <GoogleMap
              className="w-full h-96"
