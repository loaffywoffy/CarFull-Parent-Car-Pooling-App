...
export default function CarpoolList({ partyGroupId, onRequestSpot, onOfferRide, selectedCarpoolId }: CarpoolListProps) {
        ...
        const updatedCarpools = await Promise.all(
          carpools.map(async (carpool) => {
            ...
            try {
              ...
              console.log(`Carpool ${carpool.id} distances calculated:`, {
                distance,
                distanceFromUser,
  ...
  const CarpoolCard = ({ carpool }: { carpool: any }) => {
    ...
    const { data: carpoolRequests = [] } = useQuery({
      ...
      refetchInterval: 10000, // Refresh every 10 seconds to prevent rate limiting
    ...
    return (
      <Card key={carpool.id} className="mb-4 hover:shadow-lg transition-shadow overflow-hidden">
        <CardContent className="p-3 sm:p-4 md:p-6">
          ...
          {/* Enhanced Driver Route Summary - only show when viewing carpool details */}
